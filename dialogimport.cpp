...
void DialogImport::Initialize(LImageFactory::Type imageType, LColorList* lst)
{
    ...
    m_colorList->CreateUI(ui->layoutColors,0);
    m_colorList->FillComboBox(ui->cmbForeground);
    m_colorList->FillComboBox(ui->cmbBackground);

    //QObject::connect(this, LColorList::colorValueChanged, UpdateOutput);
    connect(m_colorList, SIGNAL(colorValueChanged()), this, SLOT(UpdateOutput()));
...
//    qDebug()<< "Type" << m_imageType;
    ...
    m_image->Clear();
    m_image->fromQImage(m_output.m_qImage, *m_colorList);

    if (m_output.m_qImage==nullptr)
         m_output.m_qImage = new QImage(320,200, QImage::Format_ARGB32);

    m_image->ToQImage(*m_colorList,m_output.m_qImage,1, QPoint(0,0));
